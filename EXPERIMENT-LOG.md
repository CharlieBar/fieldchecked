...
| 2026-08-13 | Plausible analytics wired site-wide as plain `<script>` tags in `<head>` — not `next/script`, which would pull its own runtime into the bundle. Measured: per-route JS unchanged at 210 B, First Load unchanged at 106 kB. **Corrected 2026-08-13** to Plausible's current issued snippet: per-site script ID in the URL rather than shared `script.js` + `data-domain`, plus the inline init that buffers events fired before the async script lands. Env var renamed `PLAUSIBLE_DOMAIN` → `PLAUSIBLE_SCRIPT_ID`, since the domain is no longer passed to the script at all. JS bundle unchanged by the correction; page HTML grew 411 B. | GSC and Plausible answer different halves of the same question and neither substitutes for the other. GSC covers everything up to the click — impressions, queries, position — and goes silent at the moment of arrival. Plausible covers everything after it. A page can win impressions and lose readers, or the reverse, and only both instruments together distinguish those. | Sessions, entry pages, and bounce/engagement per pillar, read against GSC impressions for the same URLs. | _pending live confirmation_ |
